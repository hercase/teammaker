...
const ShirtIcon: FC<ShirtIconProps> = ({ color = "#2C3590", size = 42, ...rest }) => {
  const isDarkMode = usePrefersColorScheme({ ssr: true }) === "dark";
  const mainColor = isDarkMode ? tinycolor(color).lighten(10) : tinycolor(color).darken(15);
  const contrastColor = mainColor.isDark() ? mainColor.brighten(30) : mainColor.darken(50);
  const fillColor = contrastColor.desaturate(20).toHexString();
